...
void IComSysMKVToolNix::ShowMsgRequiresMKVToolNix()
{
    ...
    QString qstrAfterInstall = tr("After installing MKVToolNix please restart the program to have it auto-detect.<br>Alternatively you can specify your MKVToolNix path in the Preferences.");

    #if defined(Q_OS_WIN) || defined(Q_OS_MACOS)
    QString qstrMKVToolNixRequired = tr("This program requires MKVToolNix, which can be downloaded from:");

    qmbMKVToolNixMsg.setText(QString("<p>%1</p>"
                                     "<p><center><a href='https://MKVToolNix.download/downloads.html'>https://MKVToolNix.download/downloads.html</a></center></p>"
                                     "<p>%2</p>")
                                     .arg(qstrMKVToolNixRequired)
                                     .arg(qstrAfterInstall));
    #else
    QString qstrMKVToolNixRequired = tr("This program requires MKVToolNix, which can be installed with the below commands:");
    QString qstrOtherDistros = tr("For other distributions, see:", "Distributions, as in Linux distributions");
    qmbMKVToolNixMsg.setText(QString("<p>%1</p>"
                                     "<p>"
                                        "Debian, Mint, Ubuntu: <font color=red>sudo apt-get install mkvtoolnix</font><br>"
                                        "Arch, Manjaro, Antergos: <font color=red>sudo pacman -S mkvtoolnix-cli</font><br>"
                                        "OpenSUSE: <font color=red>sudo zypper install mkvtoolnix</font><br>"
                                        "Fedora: <font color=red>sudo dnf install mkvtoolnix</font><br>"
                                        "%2 <a href='https://MKVToolNix.download/downloads.html'>https://MKVToolNix.download/downloads.html</a>"
                                     "</p>"
                                     "<p>%3</p>")
                                     .arg(qstrMKVToolNixRequired)
                                     .arg(qstrOtherDistros)
                                     .arg(qstrAfterInstall));
    #endif
