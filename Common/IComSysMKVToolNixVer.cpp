...
void IComSysMKVToolNixVer::MKVMergeFinished(const int kiExitCode, const QProcess::ExitStatus kqpesExitStatus)
{
    ...
    {
        QMessageBox::critical(m_pmwMainWindow, tr("Error Determining MKVToolNix Version"), tr("An error occurred while attempting to determine MKVToolNix version."), QMessageBox::Ok);
    }
    else
    {
        int iMajorVersion = 0;
...
void IComSysMKVToolNixVer::MKVMergeErrorText()
{
    ...
    QMessageBox::critical(m_pmwMainWindow, tr("Error Determining MKVToolNix Version"), QString("%1:\n\n%2").arg(tr("An error occurred while attempting to determine MKVToolNix version")).arg(qstrErrorText), QMessageBox::Ok);
}
...
void IComSysMKVToolNixVer::MKVMergeError(const QProcess::ProcessError kqpeError)
{
    if (kqpeError == QProcess::FailedToStart)
    {
        QMessageBox::critical(m_pmwMainWindow,
                              ...
                              tr("An error occurred while attempting to determine MKVToolNix version.\nPlease ensure the MKVToolNix directory has been correctly specified"),
                              QMessageBox::Ok);
    }
}
