...
QString IComDlgFileProperties::GetSizeString()
{
    ...
    QString qstrBytes = QString(tr("bytes"));
    qint64 lliSize = m_qfiFileInfo.size();
    if (lliSize > kiGigabyte)
    {
        QString qstrGigabytes = QString(tr("GB", "GB as in Gigabytes"));
        return QString("%1 %2 (%L3 %4)").arg(QString::number(static_cast<double>(lliSize) / kiGigabyte, 'f', 2)).arg(qstrGigabytes).arg(lliSize).arg(qstrBytes);
    }
    else if (lliSize > kiMegabyte)
    {
        QString qstrMegabytes = QString(tr("MB", "MB as in Megabytes"));
        return QString("%1 %2 (%L3 %4)").arg(QString::number(static_cast<double>(lliSize) / kiMegabyte, 'f', 2)).arg(qstrMegabytes).arg(lliSize).arg(qstrBytes);
    }
    else if (lliSize > kiKilobyte)
    {
        QString qstrKilobytes = QString(tr("KB", "KB as in Kilobytes"));
        return QString("%1 %2 (%L3 %4)").arg(QString::number(static_cast<double>(lliSize) / kiKilobyte, 'f', 2)).arg(qstrKilobytes).arg(lliSize).arg(qstrBytes);
    }

    return QString("%L1 %2").arg(lliSize).arg(qstrBytes);
}
...
void IComDlgFileProperties::AddTagsTabs()
{
    if (IComMetaExif::FileCanContainExif(m_qfiFileInfo.suffix()))
    {
        ...
        if (pwmeExifMetaWidget != nullptr)
        {
            ...
            m_qtabwTabWidget->addTab(pwmeExifMetaWidget, tr("Exif", "Exif as in Exchangeable image file format.  Should probably remain as Exif in most languages"));
        }
    }
    ...
    if (pwmmMusicMetaWidget != nullptr)
    {
        //pwmeExifMetaWidget->setStyleSheet("QTableWidget {border: 0px;}");
        m_qtabwTabWidget->setStyleSheet("QTabWidget::pane {margin: 0px 0px 0px 0px;}");
        m_qtabwTabWidget->addTab(pwmmMusicMetaWidget, tr("Media", "Tab showing music meta information"));
    }
}
