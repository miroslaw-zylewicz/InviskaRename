...
IComDlgProgress::IComDlgProgress(IUIMainWindow* pmwMainWindow, const QString & krqstrTitle, const QString & krqstrMessage, const int kiMax, const bool kbDisplayPercent, const bool kbBarAtTop, const int kiAppearDelayMS) : QDialog(pmwMainWindow)
{
    m_bAborted = false;
...
void IComDlgProgress::UpdateTitle(const QString & krqstrTitle)
{
    setWindowTitle(krqstrTitle);
}
...
void IComDlgProgress::HideMessage()
{
    m_pqlblMessage->setVisible(false);
}


void IComDlgProgress::HideAbort()
{
    m_pqpbAbort->setVisible(false);
}
